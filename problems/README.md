...
> 纸上写完先按 [纸面自检四问](../docs/纸面自检四问.md) 自检一遍（变量 → 边界 → 返回值 → 复杂度），再上机验证。
...
| 题号 | 题目 | 类型 | 文件 | 笔记 | 完成 |
|---|---|---|---|---|---|
| 1 | 两数之和 | 数组 · 哈希 | `p001_two_sum.go` | `notes/p001-two-sum.md` | ✅ 9/22 |
| 26 | 删除有序数组中的重复项 | 数组 · 双指针 | `p026_remove_duplicates.go` | `notes/p026-remove-duplicates.md` | ✅ 9/22 |
| 27 | 移除元素 | 数组 · 双指针 | `p027_remove_element.go` | `notes/p027-remove-element.md` | ✅ 9/23 |
...
- **已用真编译器验证过**（go1.22.12 初验；go1.27.1 于 2026/9/22 复核）：
  ...
  ```
- 上面的「完成」列由你自己勾（**已勾：题 1、26（9/22）、27（9/23）**）。
...
| 题目 | 首次日期 | 结果 | 重做日期 | 结果 |
|---|---|---|---|---|
| 1. 两数之和 | 9/22 | 独立 AC（暴力 O(n²)，三组示例实跑通过） | 9/23 ⬜ 写哈希版 O(n) |  |
| 26. 删除有序数组中的重复项 | 9/22 | 独立 AC（快慢双指针 O(n)，边界实跑全过） | 9/26 ⬜ |  |
| 27. 移除元素 | 9/23 | 独立 AC（nums[:0]+append 原地过滤，12 组边界全过） | 9/26 ⬜ |  |
|  |  |  |  |  |
|  |  |  |  |  |
|  |  |  |  |  |
|  |  |  |  |  |
