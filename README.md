...
> ✅ 本仓库已用 **go1.22.12 与 go1.27.1 两个版本**分别验证过：`go vet ./...`、`go build ./...`、`go test ./...`（25 个测试函数 / 296 个子测试 / 覆盖率 100%）全部通过，12 个语法示例全部 `go run` 成功 —— 见 [docs/验证记录.md](docs/验证记录.md)。
...
| 时间 | 内容 | 产出 |
|---|---|---|
| 周一 08:00–11:00 | 装环境 + `go mod init` + 跑通 `go run .` / `go test ./...` | 环境清单全打勾 |
| 周一–四 08:00–11:00 | 语法精读 3 块 × 50 分钟，每块跟着 `syntax/` 敲一遍 | 12 个语法点过完 |
| 周一–四 14:00–15:30 | 刷题 2 道 + 写题解 + 重做昨日错题 | 8 题/周 |
| 周五 全天 | 把本周语法点整理成可运行示例，提交 | 一次整洁的提交 |
| 周六 08:00–12:00 | 标准库速览 `fmt strings strconv sort os bufio` | `syntax/12_stdlib` |
| 周日 08:00–12:00 | 复盘 + 补欠账 + 预习 W2 | `notes/04` 收尾 |
